...
Transformer-based methods have achieved state-of-the-art (SOTA) results in various tasks. Lots of research endeavors, 
nowadays, the first thing on the list is to find a data representation that Transformer can process since it 
only takes a sequence of tokens inherently, this blog won't go into much detail into the tokenization process as it's still an 
open question about what's the most effective way to preprocess and tokenize a certain data representation. I'd like to 
talk about in my recent research work [*ODTFormer*](https://jerrygcding.github.io/odtformer/), how I formulate the problem of 
3D computer vision, specifically depth estimation and reconstruction, into Transformer-acceptable form with an intuitive
...
But in real-world applications, several other factors exist, like imperfect camera parameters, rectification or lens 
...
Let's simplify the problem to only about stereo reconstruction when only have two images of different views, 
...
Reflecting on this process, it's an exhaustive search algorithm that iterates through all possible disparities where some 
...
Almost all prior works under the stereo setting use the cost volume construction method since it's the most easy application
...
certainly more than horizontal shifts in some camera pairs under BEV and how to effectively construct cost volume with 
...
The Multiscale Deformable Attention mechanism excels at both task performance and computational complexity because it's 
...
problem with a Transformer-based architecture. However, after various experiments of directly using the MDA following 
...
uses cost volume construction and Convolutions, in stereo occupancy detection by a great margin. Only when we use a 
modified version of 3D MDA and attending to a cost volume shows some improvements in metric measurements while still 
unable to get close to StereoVoxelNet.

These phenomena makes us think that matching cost could be an essential inductive bias in 3D reconstruction, but with the 
...
bias into the Transformer-based model and against the conventional cost volume construction, called Deformable Matching Cost 
(DMC) Block.
...
sampling offsets constrained by the target voxel size to get 3D sampling locations, and the locations are then projected 
to 2D corresponding points and get the features. Moreover, to further enforce feature correspondences, we don't use 
different sampling locations for each scale, but directly scale the sampling coordinates so that they always correspond 
...
and the effectiveness of our DMC block design. The best part of it is that the theoretical application cases can be 
...
point of how to reformulate the problem representation of the model considering certain invariants, simple yet intuitive
designs can also greatly boost the model performance without affecting computational efficiency which can be useful for
improving model generalizability across different domains and settings, as well as its affordability on edge devices.
